...
tmux new-session \; \
	...
	select-pane -t 0 \; \
	send-keys "ssh -t ${MACHINES[3]} \"cd $(pwd) > /dev/null; echo -n 'Connected to '; hostname; sleep .5s; ./run_middleware.sh middleware ${MACHINES[0]} Flights ${MACHINES[1]} Cars  ${MACHINES[2]} Rooms\"" C-m \;
